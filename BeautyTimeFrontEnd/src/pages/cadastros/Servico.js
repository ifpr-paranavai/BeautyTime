...
const Servico = () => {
    ...
    const objetoService = new ServicoService();
    ...
    const onStatusChange = (e) => {
        const val = e.value === 'true'; // Convertendo o valor de string para booleano
        let _objeto = {...objeto};
        _objeto.status = val;
    ...
    const duracaoBodyTemplate = (rowData) => {
        return (
            <>
                <span className="p-column-title">Nome</span>
                {rowData.duracao} - {rowData.duracaoTipo}
            </>
        );
    }
            ...
            <div className="col-12">
                <div className="card">
                    ...
                    <DataTable ref={dt} value={objetos}
                        ...
                        <Column field="duracao" header="Duração" sortable body={duracaoBodyTemplate} headerStyle={{width: '14%', minWidth: '10rem'}}></Column>
                        <Column field="preco" header="Preço" sortable body={precoBodyTemplate} headerStyle={{width: '14%', minWidth: '10rem'}}></Column>
                        <Column body={rowData => {
                    ...
                    <Dialog visible={objetoDialog} style={{width: '450px'}} header="Cadastrar/Editar" modal className="p-fluid" footer={objetoDialogFooter} onHide={hideDialog}>
                        ...
                        <div className="field">
                            ...
                            {submitted && !objeto.name && <small className="p-invalid">Nome do serviço é obrigatório.</small>}
                        </div>
                        <div className="field flex">  {/* <-- Adicione a classe flex aqui */}
                            <div className="mr-2" style={{flex: 1}}> {/* <-- Adicione um contêiner para o InputText */}
                                <label htmlFor="duracao">Duração</label>
                                <InputText id="duracao" value={objeto.duracao} onChange={(e) => onInputChange(e, 'duracao')} required autoFocus className={classNames({'p-invalid': submitted && !objeto.duracao})}/>
                                {submitted && !objeto.duracao && <small className="p-invalid">Duração do serviço é Obrigatória.</small>}
                            </div>
                            <div style={{flex: 1}}> {/* <-- Adicione um contêiner para o Dropdown */}
                                <label htmlFor="duracaoTipo">Tempo em</label>
                                <Dropdown
                                    id="duracaoTipo"
                                    value={objeto.duracaoTipo}
                                    options={[{label: 'Minutos', value: 'minutos'}, {label: 'Horas', value: 'horas'}]}
                                    onChange={(e) => onInputChange(e, 'duracaoTipo')}
                                    placeholder="Selecione..."
                                    className={classNames({'p-invalid': submitted && !objeto.duracaoTipo})}
                                />
                            </div>
                        </div>
                        <div className="field">
                            ...
                            {submitted && !objeto.name && <small className="p-invalid">Descrição é Obrigatório.</small>}
                        </div>
                        <div className="field">
                            <label htmlFor="preco">Preço</label>
                            <InputText id="preco" value={objeto.preco} onChange={(e) => onInputChange(e, 'preco')} required autoFocus className={classNames({'p-invalid': submitted && !objeto.preco})}/>
                            {submitted && !objeto.name && <small className="p-invalid">Preço é Obrigatório.</small>}
                        </div>
                        <div className="field">
                            <label htmlFor="status">Status</label>
                            <Dropdown
                                id="status"
                                value={objeto.status.toString()}  // Convertendo o valor booleano para string
                                options={[{label: 'Ativo', value: 'true'}, {label: 'Inativo', value: 'false'}]}
                                onChange={onStatusChange}
                                placeholder="Selecione..."
                                className={classNames({'p-invalid': submitted && (objeto.status === null || objeto.status === undefined || objeto.status === '')})}
                            />
                            {submitted && (objeto.status === null || objeto.status === undefined || objeto.status === '') && <small className="p-invalid">Status é Obrigatório.</small>}
                        </div>
...
export default React.memo(Servico, comparisonFn);
